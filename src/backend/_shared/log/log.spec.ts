...
import { setupSpyLog } from './log.test_lib';
...
describe('custom log matchers', () => {
    const log = setupSpyLog();

    it('should detect multiple warning logs via matcher', () => {
        LOG.WARN('warn message');
        LOG.WARN('warn message2');

        expect(log).hasWARN('warn message');
        expect(log).hasWARN('warn message2');
    });
    it('should detect no warning logs via matcher', () => {
        // LOG.WARN('warn messageX');

        expect(log).not.hasWARN('warn message');
    });

    it('should detect info logs via matcher', () => {
        LOG.INFO('info message');

        expect(log).hasINFO('info message');
    });
});

describe('setupSpyLog()', () => {
    const log = setupSpyLog();

    it('should expose the current spy log instance', () => {
