...
export class FS {
     ...
     * @returns {string | undefined} 📤 The content of the file as a string.
     */
    static readFile(path: string, options: any = {}): string | undefined {
        let data;
    ...
    static copyFolder(source: string, target: string, recursive = true): void {
        if (!FS.exists(source)) {
            LOG.WARN(`Source file does not exist: ${source}`);
            return;
        }
        const allFiles = FS.listDetails(source, recursive);
        for (const file of allFiles) {
            if (file.type === 'file') {
                const relativePath = file.path.substring(source.length);
                ...
                if (!FS.exists(targetPath)) {
                    ...
                    LOG.INFO(`Created target folder: ${targetPath}`);
                }
            }
        }
    }
    static fileType(filePath: string): 'file' | 'folder' | 'unknown' {
        if (fs.existsSync(filePath)) {
